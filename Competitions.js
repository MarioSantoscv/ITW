...
var vm = function () {
    ...
    self.activate = function (id) {
        ...
        ajaxHelper(composedUri, 'GET').done(function (data) {
            console.log(data);
            hideLoading();
            self.athletes(data.Competitions);
...
$(document).ready(function () {
    ...
    ActiveAutocomplete(
        "#search",
        "http://192.168.160.58/Paris2024/api/Competitions/Search?q=",
        "./Competitions_details.html?id=",
        "name",
        "id"
    );
});
...
let manageFavs = function (data2) {
    let dataName = data2.Name;
    if (fav.some((item) => item.Name === dataName)) {
        fav = fav.filter((item) => item.Name !== dataName);
    } else {
        fav.push(data2);
    }
